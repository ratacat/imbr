...
class imbanditRedirector {
  ...
  // When the IMBR Admin screen save button is pressed, this code will execute.
  private function red_editor_save() {

    // 1. First delete everything from imb_linkscanner?
    $this->wpdb->query("delete from ".$this->tableLinkscanners);

    // 2. Detect if a new redirector should be created.
    ...
    } //isset($_POST['post_aff_url'])

    // 3. --------new linkscanner set to within a selection of categories----------------------------------------------------
    ...
    //-------------------------Prepare numbers for all existing boxes important-----------------------
    $redirectorCnt = $_POST['redirectorCnt'];

    // not used in the original code
    //if($redirectorCnt != 0){
    //$this->wpdb->query("delete from ".$this->wpdb->prefix . 'imb_linkscanner');
    //$this->wpdb->query("delete from ".$this->linkscannerTable);
    //}
    ...
    for ($i = 1; $i <= $redirectorCnt; $i++) {
      ...
      //$rand = "rand_" . $i;
      //$select = "select_" . $i;
      //$ls_regex_new = "ls_regex_new_" . $i;
      //--------update linkscanner set to within a selection of categories----------------------------------------------------
      ...
      // this delete should not be here, move it to imb_red_editor optional diversions
  ...
  private function resetDatabase() {
    ...
    // This function is the entry point for the functionality to display/manage
    ...
    // However, if this is a POST request then other functionality may be
    // requested, such as saving of changed options or resetting the db.
    public function imb_red_editor() {

      // Perform certain optional diversions first, and then
      // continue with the ordinary display of the admin screen.
      ...
      //  //$this->ls_init('delete');
        ...
        // If this is a POST request, and the 'UPDATE' parameter is
        // passed, then we surmise that the user has changed the options
        // and is saving them.  Hence, process the save, but then continue with ordinary
    ...
    private function imb_red_editor_center_contents() {
      ...
      $redirectorTable = $this->wpdb->prefix . 'imb_redirector';
      ...
      $redirectorsQuery = "SELECT id, random_post, random_page, url, single_pages_categories, mn, rand FROM $redirectorTable";
     ...
     if ($redirectors) { // Test for the existence of any redirectors.
        ...
        foreach ($redirectors as $redirector) {
          ...
          $adminRow->mn_numbers_div = "<label>"
            ...
            . "</label>";
          //</div> <!-- mn_numbers_div -->
          //<input name="id_<qqqphp echo $i; QQQ>" type="hidden" id="id_<qqqphp echo $i; QQQ>" value="<qqqphp echo $post_post->id; QQQ>"/>
          //<qqqphp
  ...
  private function resetRedirectors() {
    ...
    private function wp_jdis() {
      ...
      //if (!isset($level)) {
      //$this->startLevel1($mn, $redirector_table);
      //} else {  //Move to appropriate control level
    ...
    //private function startLevel1($mn, $redirector_table) {
      ...
      //$query = "select * from $redirector_table where mn='" . $mn . "'";
    ...
    private function ls_scan_all($mn, $regex = NULL, $postid = NULL,$spc = NULL) {
      ...
      $sql2Spc = "SELECT * FROM ". $tableTerms . " WHERE name in ('".$spcStr."')";
      ...
      $sql = "SELECT * FROM " . $tablePosts . " WHERE post_parent=0 AND post_status = 'publish' "
